      ...
      <el-table :data="tableBody.data">
        <el-table-column v-for="item of tableBody.header" :key="item.id" :label="item.label" :prop="item.prop">
          <template slot-scope="scope">
            <span v-if="item.prop==='type'" >{{reType(scope.row['goods'])}}</span>
            <span v-else-if="item.prop==='status'" >{{reType2(scope.row[item.prop])}}</span>
            <span v-else-if="item.prop==='goods'" >{{reType3(scope.row[item.prop])}}</span>
            <span v-else>{{scope.row[item.prop]}}</span>
          </template>
        </el-table-column>
        <el-table-column label="操作">
          <template slot-scope="scope">
            <el-button  @click="edit(scope.row)" type="text" size="small" :disabled="scope.row.status===3">编辑</el-button>
  ...
  methods: {
    ...
    search(val) {
      ...
      if(this.timeRange!=null && this.timeRange.length>0){
        params.start_time=this.timeRange[0];
