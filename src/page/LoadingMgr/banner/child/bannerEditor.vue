<template>
    <section>
        <el-form :form="form" label-width="80px" :rules="rules" :ref="form" :model="form">
            ...
            <el-form-item label="幻灯片" prop="url">
            ...
            <el-form-item label="跳转类型" prop="jump_type">
              <el-select v-model="form.jump_type" placeholder="请选择" clearable >
                <el-option
            ...
            <el-form-item label="选择时间" prop="online_at" v-if="form.status===2">
...
export default {
  ...
  methods: {
    init() {
      ...
      //获取公告下的跳转内容列表
      this.$http
        .get(`${process.env.GUESSING_HOST_URL}/api/admin/_announcement`)
        .then(resp => {
      ...
      //获取竞猜项下的跳转内容列表
      this.$http
        .get(`${process.env.GUESSING_HOST_URL}/api/admin/_item`)
        .then(resp => {
          if (resp.data.success) {
    ...
    ok(form) {
      this.$refs[form].validate(valid => {
        if (valid) {
          let url = `${process.env.GUESSING_HOST_URL}/api/admin/create_banner`,
            str = "添加",
            params = {
          ...
          if (this.type === "edit") {
            ...
            url = `${process.env.GUESSING_HOST_URL}/api/admin/edit_banner`;
            str = "编辑";
          }
