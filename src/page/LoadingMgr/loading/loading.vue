<template>
  <section v-loading="isLoad">
    ...
    <el-main>
      <el-row>
            <el-row type="flex" justify="space-between">
                ...
                <el-col :span="7">
                    <el-input style="width:80%" v-model="searchData.title" placeholder="请输入启动页标题以及关键字内容" clearable></el-input>
                </el-col>
                <el-col :span="20">
                    <el-button @click="search" type="primary" icon="el-icon-search">查询</el-button>
                    <el-button @click="openEditor" type="primary" icon="el-icon-plus">新增启动页</el-button>
                </el-col>
            </el-row>
      </el-row>
    ...
    <el-dialog :title="editorDialogTitle" :visible.sync="editorDialogVisible" width="60%">
        <loading-editor @complete="load" v-if="editorDialogVisible" :visible.sync="editorDialogVisible" :data="editData" :type='editorType'></loading-editor>
    </el-dialog>
  </section>
</template>
<script>  
import loadingEditor from "./child/loadingEditor.vue";
...
export default {
  ...
  methods: {
    ...
    search() {
      ...
      this.$http.get(`${process.env.GUESSING_HOST_URL}/api/admin/loading`, {params})
        .then(resp => {
    ...
    openEditor(val) {
      if (val.id) {
        this.editorDialogTitle = "编辑启动页";
        this.editorType = "edit";
        this.editData = val;
      } else {
        this.editorDialogTitle = "添加启动页";
        this.editorType = "add";
        this.editData = {};
      }
    ...
    remove(row) {
      this.$confirm('确认删除此条启动页?').then(_=>{
          let params={id:row.id}
          this.$http.post(`${process.env.GUESSING_HOST_URL}/api/admin/delete_loading`,params)
          .then(resp=>{
              if(resp.data.success){
