    ...
    <el-main>
      ...
      <el-row>
        ...
        <el-col :span="3" style="margin-left:20px">
            <el-select v-model="searchBox.pay_type" placeholder="支付方式" clearable @change="search">
      ...
      <el-table :data="tableBody.data" v-loading="tableBody.isLoad">
        <el-table-column v-for="item of tableBody.header" :key="item.id" :label="item.label" :prop="item.prop">
          <template slot-scope="scope">
            <span v-if="item.prop==='pay_type'">{{scope.row.pay_type | reType}}</span>
...
export default {
  ...
  filters:{
    reType(val) {return val === 1 ? "支付宝" : "微信";}
  },
  methods: {
    load() {
      this.isLoad = true;
    ...
    search() {
      ...
      if (this.searchBox.time !=null && this.searchBox.time.length>0) {
        params.start_at = this.searchBox.time[0];
        params.end_at = this.searchBox.time[1];
      }
...
const tableHeader = [
  ...
  { prop: "amount", label: "充值金额(元)" },
  {prop:"m_coin",label:"充值猫币"},
