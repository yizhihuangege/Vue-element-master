  ...
  <section>
    <el-form :form="form" label-width="100px" :rules="rules" :ref="form" :model="form" :inline-message="true" v-loading="loading">
      ...
      <el-form-item label="奖品名称" prop="prize_name">
        <el-input  v-model="form.prize_name" :maxlength="14" placeholder="奖品名称" style="max-width:300px" clearable></el-input>
      </el-form-item>
      ...
      <el-form-item label="有效时间" prop="expire_hour" v-if="form.prize_type!==4">
        <el-input-number v-model="form.expire_hour"></el-input-number> 小时
      </el-form-item>

      <el-form-item label="商品折扣" prop="discount" v-if="form.prize_type!==4">
        <el-input type="number" v-model="form.discount" placeholder="“0”表示免费，0.8代表8折" style="max-width:300px" clearable></el-input>
      </el-form-item>
      ...
      <el-form-item label="状态" prop="status">
        <el-radio-group v-model="form.status" :disabled="form.prize_type===4 || isThanks">
...
export default {
  ...
  methods: {
    ...
    //获取关联淘手游游戏列表
    getTsyGameList(){
      this.$http.get(`${process.env.GUESSING_HOST_URL}/api/admin/tsy_game`).then(resp=>{
...
const rules = {
  ...
  expire_hour: [{ required: true, message: "请输入有效时间", trigger: "blur" } ],
  ...
  tsy_goods_id: [{required: true, message: "请选择淘手游商品",trigger: "blur"}]
};
</script>
