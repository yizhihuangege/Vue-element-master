    ...
    <section>
        ...
        <el-main>
            ...
            <el-row :gutter="20">
                ...
                <el-col :span="4">
                    <el-select v-model="queryParams.game_id" placeholder="游戏名称" clearable @change="search">
                        <el-option v-for="item in gameList" :key="item.id" :label="item.name" :value="item.id"></el-option>
                    </el-select>
                </el-col>
                <el-col :span="4">
                    <el-select v-model="queryParams.status" placeholder="状态" clearable @change="search">
                ...
                <el-table-column v-for="(item,index) of tableBody.header" :key="index" :label="item.label" :prop="item.prop">
                    <template slot-scope="scope">
                        ...
                        <span v-else-if="item.prop=='item'" class="jc_item">
                            <span v-if="scope.row.item==false" @click="guessItem(scope.row)">添加</span>
        ...
        <!-- 新增/编辑 Dialog -->
        <el-dialog :title="editorDialog.title" :visible.sync="editorDialog.visible" v-if="editorDialog.visible" width="50%" :before-close="handleClose">
...
import matchEditor from "./child/matchEditor";
...
export default {
  ...
  methods: {
    ...
    //同步数据
    synchronize() {
      this.$http.post(`${process.env.GUESSING_HOST_URL}/api/admin/synchronize`).then(resp => {
          if (resp.data.success) {
    ...
    search() {
      ...
      this.$http.get(`${process.env.GUESSING_HOST_URL}/api/admin/match_list`, {params}).then(resp => {
          if (resp.data.success) {
    ...
    remove(row) {
      this.$confirm("确定删除该游戏吗?").then(_ => {
        this.$http.post(`${process.env.GUESSING_HOST_URL}/api/admin/delete_match`,{id:row.id}).then(resp => {
            if (resp.data.success) {
